# AI Gateway

AI API 聚合网关与用量计费平台学习项目。

当前实现是一个单体后端闭环：

- 用户注册、登录
- 平台 API Key 管理
- Provider Key 加密存储
- OpenAI Chat Completions、Anthropic Messages、OpenAI Responses 兼容接口
- HTTP SSE 与 Responses WebSocket 流式调用
- Provider Key 调度、调用前 failover 和健康状态
- 请求日志、流式用量、幂等计费、并发余额扣减
- Redis 限流
- 非流式成功调用的 RabbitMQ 事件发布、Topic 拓扑与手动 ACK Consumer 骨架
- MyBatis 持久化
- 单元测试、Web 测试、Mapper 集成测试

## 技术栈

- JDK 21
- Maven
- Spring Boot 3.5.x
- MyBatis
- MySQL 8.4 LTS
- Redis
- RabbitMQ / Spring AMQP
- Spring Security
- WebClient
- JUnit 5 / Mockito / Testcontainers

## 项目结构

```text
ai-gateway/
  ARCHITECTURE.md
  CODEBASE.md
  Dockerfile
  docker-compose.yml
  docs/
    requirements.md
    database.md
    api.md
    security.md
    pricing.md
    nginx-deployment.md
  nginx/
    default.conf
    https.conf.example
  sql/
    schema.sql
    data.sql
  src/main/java/com/example/aigateway/
    common/
    config/
    controller/
    dto/
    entity/
    exception/
    gateway/
    mapper/
    messaging/
    security/
    service/
    provider/
    ratelimit/
    client/
  src/main/resources/
    application.yml
    mapper/
  src/test/java/
  pom.xml
```

## 本地运行

可以用 Docker Compose 构建并启动后端、MySQL、Redis、RabbitMQ 和 Nginx：

```powershell
docker compose up -d --build
```

如果要用 Maven 直接启动后端，只启动依赖服务，再设置环境变量：

```powershell
docker compose up -d mysql redis rabbitmq
```

```powershell
$env:DB_HOST="localhost"
$env:DB_PORT="3306"
$env:DB_NAME="ai_gateway"
$env:DB_USERNAME="root"
$env:DB_PASSWORD="your_password"
$env:REDIS_HOST="localhost"
$env:REDIS_PORT="6379"
$env:RABBITMQ_HOST="localhost"
$env:RABBITMQ_PORT="5672"
$env:RABBITMQ_USERNAME="ai_gateway"
$env:RABBITMQ_PASSWORD="ai_gateway_dev"
```

RabbitMQ 管理界面为 `http://localhost:15672`，Compose 默认用户名和密码分别为 `ai_gateway`、`ai_gateway_dev`。

启动项目：

```powershell
mvn spring-boot:run
```

后端直连和经过 Nginx 的健康检查：

```powershell
curl.exe http://localhost:8080/api/health
curl.exe http://localhost:8088/api/health
```

## Nginx 与 HTTPS

Compose 当前只挂载 `nginx/default.conf`，提供 HTTP `/api/` 代理和专用 SSE 代理规则。`nginx/https.conf.example` 是不会被加载的 HTTPS 模板；域名、证书、443 端口和证书挂载准备好后才能启用。服务器同步步骤和注意事项见 [Nginx 与 HTTPS 部署](docs/nginx-deployment.md)。
